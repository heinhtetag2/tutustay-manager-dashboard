...
function RateChip({ rate, t }: { rate: RateType; t: (k: string) => string }) {
  ...
  return <span className={`inline-flex items-center px-2 py-0.5 text-[11px] font-medium rounded-full border ${cls}`}>{t(rate)} {t('rate')}</span>;
}
...
export default function BookingRequestDetail() {
  ...
  const stats = [
    { title: 'Nights', Icon: Moon, value: String(request.nights), subtitle: `${formatDate(request.checkIn)} → ${formatDate(request.checkOut)}` },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {stats.map((card, i) => (
          <motion.div key={card.title} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: i * 0.08 }} className="bg-white border border-[var(--border-default)] rounded-md p-5 flex flex-col justify-center shadow-none hover:border-[var(--brand-border)] transition-colors group">
            ...
            <div className="text-xs text-[var(--text-tertiary)] mt-2 truncate">{card.subtitle}</div>
          </motion.div>
        ))}
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <section className="bg-white border border-[var(--border-default)] rounded-md shadow-none overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5 px-6 py-5">
              ...
              <InfoRow Icon={CalendarRange} label={t('Check-in')}><span className="text-sm text-[var(--text-primary)] tabular-nums">{formatDate(request.checkIn)}</span></InfoRow>
              <InfoRow Icon={CalendarRange} label={t('Check-out')}><span className="text-sm text-[var(--text-primary)] tabular-nums">{formatDate(request.checkOut)}</span></InfoRow>
              <InfoRow Icon={Moon} label={t('Nights')}><span className="text-sm text-[var(--text-primary)] tabular-nums">{request.nights}</span></InfoRow>
              ...
              <InfoRow Icon={Clock} label={t('Requested at')}><span className="text-sm text-[var(--text-primary)] tabular-nums">{formatDateTime(request.requestedAt)}</span></InfoRow>
            </div>
          </section>
