...
function OwnerStep({ draft, set, t, showErrors, errors }: StepProps) {
  ...
  return (
    <div className="space-y-8">
      {/* Business identity — the step title already covers this, so no sub-header. */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-5 gap-y-5">
...
function AgreementStep({ draft, set, t, showErrors, errors }: StepProps) {
  const err = (k: string) => (showErrors ? errors[k] : undefined);
  const allChecked = !!draft.agreedCompliance && !!draft.confirmedAccuracy && !!draft.agreedTerms;
  const setAll = (v: boolean) => set({ agreedCompliance: v, confirmedAccuracy: v, agreedTerms: v });
  return (
    <div className="space-y-5 max-w-2xl">
      ...
      {/* Agree to all — cascades to every term below. */}
      <button
        type="button"
        onClick={() => setAll(!allChecked)}
        aria-pressed={allChecked}
        className={`w-full flex items-center gap-3 rounded-md border p-4 text-left transition-colors cursor-pointer ${
          allChecked ? 'border-[var(--brand-primary)] bg-[var(--brand-tint)]/40' : 'border-[var(--border-default)] hover:border-[var(--border-strong)]'
        }`}
      >
        <span className={`w-5 h-5 rounded-[5px] flex items-center justify-center border shrink-0 transition-colors ${allChecked ? 'bg-[var(--brand-primary)] border-[var(--brand-primary)] text-white' : 'bg-white border-[var(--border-strong)]'}`}>
          {allChecked && <Check className="w-3.5 h-3.5" strokeWidth={3} />}
        </span>
        <span className="text-sm font-semibold text-[var(--text-primary)]">{t('Agree to all')}</span>
      </button>

      {/* Individual terms */}
      <div className="rounded-md border border-[var(--border-default)] bg-white divide-y divide-[var(--border-default)]">
        <div className="p-4">
